...
var Returns = require('../models').Returns;
var Trash = require('../models').Trash;
var q = require('q');
var queue = require('../services/queue');
var debug = require('debug')('returnsController');

var service = 'Returns';

var ReturnsController = {};

ReturnsController.buildProjection = function (projections) {
...
ReturnsController.find = function (req, res, next) {
    var query;
    if (req.query.search) {
        ...
        if (projection) {
            ourProjection = ReturnsController.buildProjection(projection);
            delete query.select;
        }
        ...
        var totalResult = Returns.estimatedDocumentCount(query);
        var total = Returns.estimatedDocumentCount({});
        var question = Returns.find(query);
...
ReturnsController.findOne = function (req, res, next) {
    ...
    var question = Returns.findById(id);
    if (populate) {
        delete query.populate;
        question = question.populate(populate);
    }
...
ReturnsController.create = function (req, res, next) {
    var data = req.body;
    if (data && data.secure) {
        delete data.secure;
    }
...
ReturnsController.update = function (req, res, next) {
    var query = req.query;
    // Clean appId and userId
    if (query && query.appId) {
        delete query.appId;
...
ReturnsController.updateOne = function (req, res, next) {
    var id = req.params.id;
    var data = req.body;
    if (data && data.secure) {
        delete data.secure;
    ...
    Returns.findByIdAndUpdate(id, data)
        .then(function (resp) {
            if (!resp) {
                next();
            } else {
...
ReturnsController.delete = function (req, res, next) {
    var query = req.query;
    // Clean appId and userId
    if (query && query.appId) {
        delete query.appId;
    ...
    Returns.find(query)
        ...
        .then(function (resp) {
            // Delete matches
            return [Returns.deleteMany(query), resp];
        })
...
ReturnsController.deleteOne = function (req, res, next) {
    var id = req.params.id;
    // Find match
    Returns.findById(id)
        .then(function (resp) {
        ...
        .then(function (resp) {
            // Delete match
            return [Returns.findByIdAndRemove(id), resp];
        })
        .spread(function (deleted, resp) {
            if (!resp) {
                next();
...
ReturnsController.restore = function (req, res, next) {
    var id = req.params.id;
    // Find data by ID from trash 
    Trash.findById(id)
        .then(function (resp) {
            // Restore to DB
            return Returns.create(resp.data);
        })
        .then(function (resp) {
            // Delete from trash
