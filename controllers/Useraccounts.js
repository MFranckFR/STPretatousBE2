...
var debug = require('debug')('useraccountsController');

var service = 'Useraccounts';

var UseraccountsController = {};

UseraccountsController.buildProjection = function (projections) {
...
UseraccountsController.find = function (req, res, next) {
    var query;
    if (req.query.search) {
        ...
        if (projection) {
            ourProjection = UseraccountsController.buildProjection(projection);
            delete query.select;
        }
        ...
        var totalResult = Useraccounts.estimatedDocumentCount(query);
        var total = Useraccounts.estimatedDocumentCount({});
        var question = Useraccounts.find(query);
...
UseraccountsController.findOne = function (req, res, next) {
    ...
    var question = Useraccounts.findById(id);
    if (populate) {
        delete query.populate;
        question = question.populate(populate);
    }
...
UseraccountsController.create = function (req, res, next) {
    var data = req.body;
    if (data && data.secure) {
        delete data.secure;
    }

    Useraccounts.create(data)
        .then(function (resp) {
            res.ok(resp);
        })
        .catch(function (err) {
...
UseraccountsController.update = function (req, res, next) {
    var query = req.query;
    // Clean appId and userId
    if (query && query.appId) {
        delete query.appId;
    ...
    Useraccounts.updateMany(query, data)
        .then(function (resp) {
            res.ok(resp);
        })
        .catch(function (err) {
...
UseraccountsController.updateOne = function (req, res, next) {
    var id = req.params.id;
    var data = req.body;
    if (data && data.secure) {
        delete data.secure;
    ...
    Useraccounts.findByIdAndUpdate(id, data)
        .then(function (resp) {
            if (!resp) {
                next();
            } else {
...
UseraccountsController.delete = function (req, res, next) {
    var query = req.query;
    // Clean appId and userId
    if (query && query.appId) {
        delete query.appId;
    ...
    Useraccounts.find(query)
        ...
        .then(function (resp) {
            // Delete matches
            return [Useraccounts.deleteMany(query), resp];
        })
...
UseraccountsController.deleteOne = function (req, res, next) {
    var id = req.params.id;
    // Find match
    Useraccounts.findById(id)
        ...
        .then(function (resp) {
            // Delete match
            return [Useraccounts.findByIdAndRemove(id), resp];
        })
        .spread(function (deleted, resp) {
            if (!resp) {
                next();
...
UseraccountsController.restore = function (req, res, next) {
    var id = req.params.id;
    // Find data by ID from trash 
    Trash.findById(id)
        .then(function (resp) {
            // Restore to DB
            return Useraccounts.create(resp.data);
        })
        .then(function (resp) {
            // Delete from trash
