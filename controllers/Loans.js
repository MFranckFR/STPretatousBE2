...
var debug = require('debug')('loansController');

var service = 'Loans';

var LoansController = {};

LoansController.buildProjection = function (projections) {
...
LoansController.find = function (req, res, next) {
    var query;
    if (req.query.search) {
        ...
        if (projection) {
            ourProjection = LoansController.buildProjection(projection);
            delete query.select;
        }
        ...
        var totalResult = Loans.estimatedDocumentCount(query);
        var total = Loans.estimatedDocumentCount({});
        var question = Loans.find(query);
...
LoansController.findOne = function (req, res, next) {
    ...
    var question = Loans.findById(id);
    if (populate) {
        delete query.populate;
        question = question.populate(populate);
    }
...
LoansController.create = function (req, res, next) {
    var data = req.body;
    if (data && data.secure) {
        delete data.secure;
    }
...
LoansController.update = function (req, res, next) {
    var query = req.query;
    // Clean appId and userId
    if (query && query.appId) {
        delete query.appId;
...
LoansController.updateOne = function (req, res, next) {
    var id = req.params.id;
    var data = req.body;
    if (data && data.secure) {
        delete data.secure;
    ...
    Loans.findByIdAndUpdate(id, data)
        .then(function (resp) {
            if (!resp) {
                next();
            } else {
...
LoansController.delete = function (req, res, next) {
    var query = req.query;
    // Clean appId and userId
    if (query && query.appId) {
        delete query.appId;
    ...
    Loans.find(query)
        ...
        .then(function (resp) {
            // Delete matches
            return [Loans.deleteMany(query), resp];
        })
...
LoansController.deleteOne = function (req, res, next) {
    var id = req.params.id;
    // Find match
    Loans.findById(id)
        .then(function (resp) {
        ...
        .then(function (resp) {
            // Delete match
            return [Loans.findByIdAndRemove(id), resp];
        })
        .spread(function (deleted, resp) {
            if (!resp) {
                next();
...
LoansController.restore = function (req, res, next) {
    var id = req.params.id;
    // Find data by ID from trash 
    Trash.findById(id)
        .then(function (resp) {
            // Restore to DB
            return Loans.create(resp.data);
        })
        .then(function (resp) {
            // Delete from trash
