...
var debug = require('debug')('productsController');

var service = 'Products';

var ProductsController = {};

ProductsController.buildProjection = function (projections) {
...
ProductsController.find = function (req, res, next) {
    var query;
    if (req.query.search) {
        ...
        if (projection) {
            ourProjection = ProductsController.buildProjection(projection);
            delete query.select;
        }
        ...
        var totalResult = Products.estimatedDocumentCount(query);
        var total = Products.estimatedDocumentCount({});
        var question = Products.find(query);
...
ProductsController.findOne = function (req, res, next) {
...
ProductsController.update = function (req, res, next) {
    var query = req.query;
    // Clean appId and userId
    if (query && query.appId) {
        delete query.appId;
...
ProductsController.updateOne = function (req, res, next) {
    var id = req.params.id;
    var data = req.body;
    if (data && data.secure) {
        delete data.secure;
...
ProductsController.delete = function (req, res, next) {
    ...
    Products.find(query)
        ...
        .then(function (resp) {
            // Delete matches
            return [Products.deleteMany(query), resp];
        })
...
ProductsController.deleteOne = function (req, res, next) {
    var id = req.params.id;
    // Find match
    Products.findById(id)
        ...
        .then(function (resp) {
            // Delete match
            return [Products.findByIdAndRemove(id), resp];
        })
        .spread(function (deleted, resp) {
            if (!resp) {
                next();
...
ProductsController.restore = function (req, res, next) {
    var id = req.params.id;
    // Find data by ID from trash 
    Trash.findById(id)
        .then(function (resp) {
            // Restore to DB
            return Products.create(resp.data);
        })
        .then(function (resp) {
            // Delete from trash
