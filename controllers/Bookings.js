...
var debug = require('debug')('bookingsController');

var service = 'Bookings';

var BookingsController = {};

BookingsController.buildProjection = function (projections) {
...
BookingsController.find = function (req, res, next) {
    var query;
    if (req.query.search) {
        ...
        if (projection) {
            ourProjection = BookingsController.buildProjection(projection);
            delete query.select;
        }
        ...
        var totalResult = Bookings.estimatedDocumentCount(query);
        var total = Bookings.estimatedDocumentCount({});
        var question = Bookings.find(query);
...
BookingsController.findOne = function (req, res, next) {
    ...
    var question = Bookings.findById(id);
    if (populate) {
        delete query.populate;
        question = question.populate(populate);
    }
...
BookingsController.create = function (req, res, next) {
    var data = req.body;
    if (data && data.secure) {
        delete data.secure;
    }
...
BookingsController.update = function (req, res, next) {
    var query = req.query;
    // Clean appId and userId
    if (query && query.appId) {
        delete query.appId;
    ...
    Bookings.updateMany(query, data)
        .then(function (resp) {
            res.ok(resp);
        })
        .catch(function (err) {
...
BookingsController.updateOne = function (req, res, next) {
    var id = req.params.id;
    var data = req.body;
    if (data && data.secure) {
        delete data.secure;
    ...
    Bookings.findByIdAndUpdate(id, data)
        .then(function (resp) {
            if (!resp) {
                next();
            } else {
...
BookingsController.delete = function (req, res, next) {
    var query = req.query;
    // Clean appId and userId
    if (query && query.appId) {
        delete query.appId;
    ...
    Bookings.find(query)
        ...
        .then(function (resp) {
            // Delete matches
            return [Bookings.deleteMany(query), resp];
        })
...
BookingsController.deleteOne = function (req, res, next) {
    var id = req.params.id;
    // Find match
    Bookings.findById(id)
        ...
        .then(function (resp) {
            // Delete match
            return [Bookings.findByIdAndRemove(id), resp];
        })
        .spread(function (deleted, resp) {
            if (!resp) {
                next();
...
BookingsController.restore = function (req, res, next) {
    var id = req.params.id;
    // Find data by ID from trash 
    Trash.findById(id)
        .then(function (resp) {
            // Restore to DB
            return Bookings.create(resp.data);
        })
        .then(function (resp) {
            // Delete from trash
