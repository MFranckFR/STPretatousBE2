...
var debug = require('debug')('messagesController');

var service = 'Messages';

var MessagesController = {};

MessagesController.buildProjection = function (projections) {
...
MessagesController.find = function (req, res, next) {
    var query;
    if (req.query.search) {
        ...
        if (projection) {
            ourProjection = MessagesController.buildProjection(projection);
            delete query.select;
        }
        ...
        var totalResult = Messages.estimatedDocumentCount(query);
        var total = Messages.estimatedDocumentCount({});
        var question = Messages.find(query);
...
MessagesController.findOne = function (req, res, next) {
    ...
    var question = Messages.findById(id);
    if (populate) {
        delete query.populate;
        question = question.populate(populate);
    }
...
MessagesController.create = function (req, res, next) {
    var data = req.body;
    if (data && data.secure) {
        delete data.secure;
    }
...
MessagesController.update = function (req, res, next) {
    var query = req.query;
    // Clean appId and userId
    if (query && query.appId) {
        delete query.appId;
    ...
    Messages.updateMany(query, data)
        .then(function (resp) {
            res.ok(resp);
        })
        .catch(function (err) {
...
MessagesController.updateOne = function (req, res, next) {
    var id = req.params.id;
    var data = req.body;
    if (data && data.secure) {
        delete data.secure;
...
MessagesController.delete = function (req, res, next) {
    var query = req.query;
    // Clean appId and userId
    if (query && query.appId) {
        delete query.appId;
    ...
    Messages.find(query)
        ...
        .then(function (resp) {
            // Delete matches
            return [Messages.deleteMany(query), resp];
        })
...
MessagesController.deleteOne = function (req, res, next) {
    var id = req.params.id;
    // Find match
    Messages.findById(id)
        ...
        .then(function (resp) {
            // Delete match
            return [Messages.findByIdAndRemove(id), resp];
        })
        .spread(function (deleted, resp) {
            if (!resp) {
                next();
...
MessagesController.restore = function (req, res, next) {
    var id = req.params.id;
    // Find data by ID from trash 
    Trash.findById(id)
        .then(function (resp) {
            // Restore to DB
            return Messages.create(resp.data);
        })
        .then(function (resp) {
            // Delete from trash
