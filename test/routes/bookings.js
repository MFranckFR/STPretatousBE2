...
describe('/bookings', function () {
    ...
    before(function (done) { /* jslint ignore:line */
        ...
        agent.get('/initialize').then(function (resp) {
    ...
    it('should create a document', function (done) {
        agent.post('/bookings').set('x-tag', tag).send({ name: 'femi2' }).then(function (resp) {
    ...
    it('should create documents', function (done) {
        agent.post('/bookings').set('x-tag', tag).send([
            {
                name: 'femi',
                toPop: oneId
            }, {
    ...
    describe('Find', function () {
        it('should search for matching documents for a given string', function (done) {
            agent.get('/bookings?search=femi').set('x-tag', tag).expect(200, done);
        });
        it('should limit the number of returned documents', function (done) {
            agent.get('/bookings?limit=2').set('x-tag', tag).then(function (resp) {
        ...
        it('should contain count of total record for the query', function (done) {
            agent.get('/bookings').set('x-tag', tag).then(function (resp) {
        ...
        it('should sort documents in ascending order', function (done) {
            agent.get('/bookings?sort=name').set('x-tag', tag).expect(200, done);
        });
        ...
        it('should select just few parameters from the documents', function (done) {
            agent.get('/bookings?select=name').set('x-tag', tag).expect(200, done);
        });
        it('should populate data of a reference for multiple data', function (done) {
            agent.get('/bookings?populate=toPop').set('x-tag', tag).then(function (resp) {
        ...
        it('should populate data of a reference for single data', function (done) {
            agent.get('/bookings/' + oneId2 + '?populate=toPop').set('x-tag', tag).then(function (resp) {
        ...
        it('should load next page for pagination', function (done) {
            agent.get('/bookings?lastId=' + last + '').set('x-tag', tag).expect(200, done);
        });
        it('should filter by date range', function (done) {
            agent.get('/bookings?from=' + from + '&to=' + new Date().toISOString()).set('x-tag', tag).then(function (resp) {
                resp.body.data.length.should.be.above(0);
                done();
            }).catch(function (err) {
                done(err);
        ...
        it('should filter by date range without setting the end date', function (done) {
            agent.get('/bookings?from=' + from).set('x-tag', tag).then(function (resp) {
                resp.body.data.length.should.be.above(0);
                done();
            }).catch(function (err) {
                done(err);
    ...
    it('should find one document', function (done) {
        agent.get('/bookings/' + oneId + '').set('x-tag', tag).then(function (resp) {
    ...
    it('should update documents', function (done) {
        agent.put('/bookings?name=femi').set('x-tag', tag).send({ name: 'Bukola' }).then(function (resp) {
            resp.body.data.should.be.an('object');
            done();
        }).catch(function (err) {
            done(err);
    ...
    it('should update a document', function (done) {
        agent.patch('/bookings/' + oneId + '').set('x-tag', tag).send({ name: 'Bukola' }).then(function (resp) {
            resp.body.data.should.be.an('object');
            done();
        }).catch(function (err) {
            done(err);
    ...
    describe('Delete', function () {
        ...
        it('should delete multiple data', function (done) {
            agent.delete('/bookings?name=femi2').set('x-tag', tag).then(function (resp) {
        ...
        it('should delete one data', function (done) {
            agent.delete('/bookings/' + oneId + '').set('x-tag', tag).then(function (resp) {
                resp.body.data.should.be.an('object');
                done();
            }).catch(function (err) {
                done(err);
    ...
    describe('Restore', function () {
        it('should restore a previously deleted data', function (done) {
            ...
            setTimeout(function () {
                Trash.findOne({ 'data._id': tid }).then(function (resp) {
                    return agent.post('/bookings/' + resp._id + '/restore').set('x-tag', tag);
                }).then(function (resp) {
                    resp.body.data.should.be.an('object');
                    done();
                }).catch(function (err) {
