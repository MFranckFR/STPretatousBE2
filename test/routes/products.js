...
describe('/products', function () {
    ...
    describe('Find', function () {
        ...
        it('should populate data of a reference for single data', function (done) {
            agent
                .get('/products/' + oneId2 + '?populate=toPop')
                .set('x-tag', tag)
                .then(function (resp) {
        ...
        it('should load next page for pagination', function (done) {
            agent
                .get('/products?lastId=' + last + '')
                .set('x-tag', tag)
                .expect(200, done);
        });
        it('should filter by date range', function (done) {
            agent
                .get('/products?from=' + from + '&to=' + new Date().toISOString())
                .set('x-tag', tag)
                .then(function (resp) {
                    resp.body.data.length.should.be.above(0);
                    done();
        ...
        it('should restore a previously deleted data', function (done) {
            ...
            setTimeout(function () {
                Trash.findOne({ 'data._id': tid })
                    .then(function (resp) {
                        return agent
                            .post('/products/' + resp._id + '/restore')
                            .set('x-tag', tag);
                    })
                    .then(function (resp) {
                        resp.body.data.should.be.an('object');
