...
describe('/returns', function () {
    ...
    describe('Find', function () {
        ...
        it('should populate data of a reference for single data', function (done) {
            agent
                .get('/returns/' + oneId2 + '?populate=toPop')
                .set('x-tag', tag)
                .then(function (resp) {
        ...
        it('should load next page for pagination', function (done) {
            agent
                .get('/returns?lastId=' + last + '')
                .set('x-tag', tag)
                .expect(200, done);
        });
        it('should filter by date range', function (done) {
            agent
                .get('/returns?from=' + from + '&to=' + new Date().toISOString())
                .set('x-tag', tag)
                .then(function (resp) {
                    resp.body.data.length.should.be.above(0);
                    done();
